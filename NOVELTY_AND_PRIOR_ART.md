...
| Literal quartic rows give the exact, residue-determined \(1\)-or-\(p^2\) congruence modulo \(\operatorname{lcm}(\operatorname{ord}(\zeta_5-1),5)\) | Lean-checked end to end | `localS5_modEq_frobenius_power_lcm` compares a literal local row with the matching Frobenius-power row, while `inertQuotient_residue_determined` and `determinedQuarticOrderRows_of_skeleton` label \(j=0\) or \(2\) exactly as in the paper. No claim of mathematical priority is made. The next common-root step is now formalized separately; the \(q^4\) norm and complete finite-fiber theorem remain paper-level. |
| Literal inert final row to explicit resultant divisibility | Lean-checked end to end | `pure_row_dvd_resultant` and `twisted_row_dvd_resultant` prove that the concrete pure/twisted `LocalS5` row supplies a common root modulo \(q\), hence \(q\) divides the corresponding integer resultant. This uses the separately checked irreducibility of \(\Phi_5\) in the inert branch. No mathematical priority claim is made. The stronger \(q^4\) norm divisibility, nonvanishing, effective bound and global finite-fiber assembly remain paper-level. |
...
- the \(q^4\) inert-norm, nonvanishing, effective bound and full
  finite-fiber assembly in the three-factor case (the preceding
  row-to-\(q\)-divisibility resultant trap is now Lean-checked);
