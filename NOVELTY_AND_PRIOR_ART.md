...
| Order-four local rows are automatically odd; concrete squarefree reduction to the quartic skeleton | Lean-checked end to end | `localS5_orderFour_odd` closes the parity seam by a conjugate-row argument. `squarefree_counterexample_concrete_dichotomy` produces either the H4 witness or the full inert skeleton with odd cardinality at least three. `explicitResultantRows_of_skeleton` now continues that skeleton to the terminal pure/twisted resultant rows. Mathematical priority is unassessed; compatible-prefix CRT sufficiency and fiber emptiness remain paper-level/open. |
| Literal quartic rows give the exact, residue-determined \(1\)-or-\(p^2\) congruence modulo \(\operatorname{lcm}(\operatorname{ord}(\zeta_5-1),5)\) | Lean-checked end to end | `localS5_modEq_frobenius_power_lcm` compares a literal local row with the matching Frobenius-power row, while `inertQuotient_residue_determined` and `determinedQuarticOrderRows_of_skeleton` label \(j=0\) or \(2\) exactly as in the paper. No claim of mathematical priority is made. |
| Literal inert final row to a fixed nonzero resultant and closed fourth-power size trap | Lean-checked end to end | `pure_row_pow_four_dvd_resultant` and `twisted_row_pow_four_dvd_resultant` give \(q^4\)-divisibility. The mod-\(5\) theorems prove \(\operatorname{Res}\equiv1\pmod5\), while `pure_resultant_natAbs_le`, `twisted_resultant_natAbs_le` and their row corollaries give \(q^4\le|\operatorname{Res}|\le16\cdot5^A\). `explicitResultantRows_of_skeleton` assembles the correct branch at every skeleton factor. No mathematical priority claim is made; universal fiber emptiness remains open. |
| Concrete final-row gap and size obstruction on the quartic skeleton | Lean-checked end to end | `quarticSkeleton_orderModulus_le_gap` and `quarticSkeleton_factor_size_exclusion` connect the literal skeleton modulus to the exact pure/twisted gap lemmas. This is the universal theorem behind the finite size sieve; it neither supplies an asymptotic order lower bound nor proves the residual finite cases empty. Mathematical priority is unassessed. |
| Exact binary order law and labelled dyadic rays | Lean-checked end to end | `CyclotomicDyadic.lean` derives \(v_2(T_p)=v_2(p^2-1)+1\) from the classical Frobenius quotient of exact order ten. `DyadicTriangle.lean` then proves that a complete three-row triangle has one common depth and that the pure/twisted labels give the exact half-modulus translations modulo \(2^{b+1}\). The underlying Frobenius identity is in Lenstra–Pomerance and Váňa; priority of the exact valuation and triangle classification is unassessed. This is a classification of the binary projection, not an additional sieve after the triangle and not a proof of odd-tail incompatibility. |
| Canonical odd-tail incidence triangle | Lean-checked end to end | `OddTailTriangle.lean` removes the full \(2\)- and \(5\)-primary parts from the concrete quartic modulus, proves the remainder divides \(p^2-1\), and derives all three pairwise support-gap divisibilities and their sharp size bounds. These are exact consequences of the local rows and give an executable oversized-tail rejection certificate. Mathematical priority is unassessed. No claim is made that one tail is universally oversized; that multiplicative-order lower-bound problem remains open. |
| Exact norm-visible two-jaw decomposition and cross-sign partition | Lean-checked end to end | `QuarticNormJaw.lean` proves the literal norm identity \((\zeta_5-1)^{1+p+p^2+p^3}=5\), identifies the complete \(p-1\) jaw with the odd scalar-five order, factors the full canonical tail into coprime \(p-1\) and \(p+1\) jaws, and proves that shared odd support cannot change sign across incident rows. The abstract forced-partition mechanism is classical and appears in the Williams/McIntosh setting; no novelty claim is made for that principle. Priority of this exact cyclotomic-order realization is unassessed. It is a structural sieve, not a proof of universal incompatibility. |
| Exact signed incidence and norm-kernel factorization | Lean-checked end to end | `QuarticNormJaw.lean` proves \(T_p=\operatorname{ord}_p(5)\gcd(T_p,1+p+p^2+p^3)\). `QuarticSignedIncidence.lean` proves that the complete shared tail is exactly the product of the minus-minus and plus-plus gcds, and that this product divides the prime gap. These are exact refinements of the classical support-partition mechanism. Their priority is unassessed, and no lower bound forcing a nontrivial or oversized overlap is claimed. |
...
| Four-coefficient primitive support and five single-odd-support depth exclusions | Intrinsic bridge and the \(p-1=2^bq^e\), \(b\in\{3,4,5,6\}\), exclusions Lean-checked; original integral \(G_{r,s}\)-bridge paper-audited | The kernel defines the exact support \(\gcd(c_{r,s},d_{r,s},U_k+1,U_{k-1}+1)\), checks both components above a split prime, and derives the exact scalar orders from its good divisors. `SingleSupportExclusion.lean` proves five complete infinite-family exclusions. The depth-five exceptional factor \(11489\) is eliminated by an exact semiperiod certificate, and depth six by the non-prime-power cofactor \(459735\). The remaining formal boundary is the paper's aurean integer \(B_s\) and its exact-level identification with the original cyclotomic gcd \(G_{r,s}\). No exact match was found in the targeted sources, but priority remains provisional pending specialist review and a complete database search. These are reductions of the H4 search space, not a proof of H4; no claim is made for \(b\ge8\). |
| 5-smooth split H4 support is impossible | Lean-checked end to end | `FiveSmoothSupportExclusion.lean` proves that no good primitive split divisor in the \(p\equiv1\pmod5\) branch can satisfy \(p-1=2^b5^f\). The argument is elementary once the exact scalar orders are available: opposite parity forces one semiorder to be \(1\), which would give an impossible order-two scalar. Priority is unassessed. This excludes an infinite family but does not prove H4. |
...
- compatible-prefix CRT sufficiency and universal fiber emptiness (the
  skeleton-to-terminal-resultant assembly and the closed-form archimedean
  bound are now Lean-checked);
