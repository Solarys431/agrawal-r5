...
| Order-four local rows are automatically odd; concrete squarefree reduction to the quartic skeleton | Lean-checked end to end | `localS5_orderFour_odd` closes the parity seam by a conjugate-row argument. `squarefree_counterexample_concrete_dichotomy` now produces either the H4 witness or the full inert skeleton with odd cardinality at least three. Mathematical priority is unassessed; the later skeleton-to-fiber step remains paper-level. |
| Literal quartic rows give the exact, residue-determined \(1\)-or-\(p^2\) congruence modulo \(\operatorname{lcm}(\operatorname{ord}(\zeta_5-1),5)\) | Lean-checked end to end | `localS5_modEq_frobenius_power_lcm` compares a literal local row with the matching Frobenius-power row, while `inertQuotient_residue_determined` and `determinedQuarticOrderRows_of_skeleton` label \(j=0\) or \(2\) exactly as in the paper. No claim of mathematical priority is made. The resultant step is now formalized separately; nonvanishing, effective bounds and complete finite-fiber assembly remain paper-level. |
| Literal inert final row to fourth-power resultant divisibility | Lean-checked end to end | `pure_row_pow_four_dvd_resultant` and `twisted_row_pow_four_dvd_resultant` prove that the concrete pure/twisted `LocalS5` row forces \(q^4\) to divide the corresponding integer resultant. The generic theorem `prime_pow_natDegree_dvd_resultant_of_common_root` extracts the full degree contribution from irreducibility, coefficientwise remainder divisibility and resultant homogeneity. No mathematical priority claim is made. Nonvanishing, the effective bound and global finite-fiber assembly remain paper-level. |
...
- nonvanishing, the effective bound and full finite-fiber assembly in the
  three-factor case (the preceding literal row-to-\(q^4\)-divisibility
  resultant trap is now Lean-checked);
