...
| Literal quartic rows give the exact \(1\)-or-\(p^2\) congruence modulo \(\operatorname{lcm}(\operatorname{ord}(\zeta_5-1),5)\) | Lean-checked end to end | `localS5_modEq_frobenius_power` compares a literal local row with the matching Frobenius-power row; `quarticOrderRows_of_skeleton` assembles the exact row at every factor. This formalizes the rigidity interface used by the paper. No claim of mathematical priority is made, and the subsequent finite-fiber construction remains paper-level. |
