...
| Literal quartic rows give the exact, residue-determined \(1\)-or-\(p^2\) congruence modulo \(\operatorname{lcm}(\operatorname{ord}(\zeta_5-1),5)\) | Lean-checked end to end | `localS5_modEq_frobenius_power_lcm` compares a literal local row with the matching Frobenius-power row, while `inertQuotient_residue_determined` and `determinedQuarticOrderRows_of_skeleton` label \(j=0\) or \(2\) exactly as in the paper. No claim of mathematical priority is made. The resultant step is now formalized separately; the closed-form archimedean estimate and complete global finite-fiber assembly remain paper-level. |
| Literal inert final row to a fixed nonzero resultant and fourth-power size trap | Lean-checked end to end | `pure_row_pow_four_dvd_resultant` and `twisted_row_pow_four_dvd_resultant` give \(q^4\)-divisibility. `pure_resultant_mod_five_eq_one` and `twisted_resultant_mod_five_eq_one` prove the stronger nonvanishing certificate \(\operatorname{Res}\equiv1\pmod5\), while `pure_row_pow_four_le_resultant_natAbs` and its twisted companion give \(q^4\le|\operatorname{Res}|\). No mathematical priority claim is made. A closed-form archimedean estimate and global tuple assembly remain paper-level. |
...
- the closed-form archimedean bound and full global finite-fiber assembly in
  the three-factor case (the literal row-to-fixed-nonzero-resultant bound is
  now Lean-checked);
