...
6. checks the third side of the CRT triangle modulo \(\gcd(T_p,T_r)\);
7. checks the two universal local-row size bounds;
8. solves the exact reduced linear congruence in the multiplier \(h\);
9. checks that the linear normal form agrees with the original row.
...
pass the complete CRT triangle       17
pass both local-row size bounds       1
pass triangle and size together       0
