...
\section{The kernel form of the golden-inertia wall}

The arithmetic reduction underlying the first open problem is now also
kernel-checked. Recall the definitions of \(A_n\) and \(H_n\) from
Section~1.

\begin{theorem}[kernel-checked local-to-arithmetic bridge]
\label{thm:localH}
Let \(p\ne5\) be prime, let
\[
 T_p=(\mathbf Z/p\mathbf Z)[X]/(\Phi_5),\qquad \zeta=X\bmod\Phi_5,
\]
and let \(n\ge1\), \(n\equiv4\pmod5\). If, for \(m=2n-1\),
\[
 (\zeta-1)^m=\zeta^m-1\qquad\text{in }T_p,
\]
then \(p\mid H_n\).
\end{theorem}
...
\begin{center}
\small
\lean{LocalS5},\quad \lean{OrderFourTransportWitness},\\
\lean{orderFourTransport\_rows},\\
\lean{orderFourTransport\_dvd\_goldenH}.
\end{center}
They start from the literal local row, evaluate its polynomial remainder at
\(\zeta,\zeta^2,\zeta^4,\zeta^3\), and recover the labelled cycle
\[
u_1\longmapsto u_2\longmapsto u_4\longmapsto u_3\longmapsto u_1.
\]
The cycle implies, without division,
\(\eps^{2n}=-1\) and \(5^{n-1}=-1\) in the cyclotomic quotient; the
Fibonacci--Lucas recurrences then descend these identities to
\(p\mid H_n\).

\begin{theorem}[kernel-checked scalar completeness]
\label{thm:scalarcomplete}
Let \(p\ne2,5\) be prime. Then the following are equivalent:
\begin{enumerate}
\item there exists \(n\ge1\), \(n\equiv4\pmod5\), such that, with
      \(m=2n-1\),
      \[
      (\zeta-1)^m=\zeta^m-1\qquad\text{in }T_p;
      \]
\item there exists \(n\ge1\), \(n\equiv4\pmod5\), such that \(p\mid H_n\).
\end{enumerate}
\end{theorem}

The converse is existential: it does not claim that the same index works.
Starting from \(p\mid H_n\), define the common cyclotomic defect
\[
 w=\frac{(\zeta-1)^{2n-1}}{\zeta^2-1}.
\]
The formal proof avoids division by representing the denominator as a
unit. It proves that \(w\) is fixed by \(\zeta\mapsto\zeta^2\) and
\(w^2=1\). A trace argument, valid also when \(\Phi_5\) splits modulo
\(p\), gives \(w=1\) or \(w=-1\).  The positive sign yields the original
index.  For the negative sign, putting \(m=2n-1\) and
\[
 d=\left\lfloor\frac{m^2}{2}\right\rfloor
   =\frac{m^2-1}{2},
 \qquad n'=n+5d,
\]
one obtains
\[
 (\zeta-1)^{\,2n'-1}=\zeta^{\,2n'-1}-1,
 \qquad n'\equiv4\pmod5.
\]
Thus the sign ambiguity is repaired constructively.

The declarations are
\begin{center}
\small
\lean{commonDefect\_eq\_one\_or\_neg\_one},\\
\lean{localS5\_sign\_repair},\\
\lean{dvd\_goldenH\_imp\_hasOrderFourTransport},\\
\lean{hasOrderFourTransport\_iff\_goldenH\_support}.
\end{center}
This theorem closes the local scalar-completeness reduction. It does
\emph{not} prove golden inertia: the universal exclusion of split prime
factors from the support of \(H_n\) remains Conjecture~\ref{conj:inertia}.

\begin{theorem}[kernel-checked scalar profile]
\label{thm:hprofile}
Let \(p\) be prime, \(p\ne5\), and \(n\ge1\). Then
\[
 p\mid H_n
...
\begin{center}
\small
\lean{dvd\_goldenH\_iff\_scalar\_profile}.
\end{center}

Both parity branches are proved in both directions. In particular, the
Fibonacci--Lucas condition is not used merely as a necessary shadow of
the scalar semiperiod.

\begin{theorem}[kernel-checked 2-adic form]
\label{thm:h4wall}
Let \(p\ne2,5\) be prime, \(n\ge2\), and \(p\mid H_n\). Then
\[
 v_2\!\left(\operatorname{ord}_p(5)\right)=v_2(n-1)+1,
\]
and
\[
 \left(\frac5p\right)=-1
...
\begin{center}
\small
\lean{dvd\_goldenH\_order\_factorization\_two}\\
\lean{dvd\_goldenH\_nonsquare\_iff\_two\_adic\_saturation}.
\end{center}

Thus the remaining universal assertion is exactly the last equality,
not the already-proved order identity. The theorem is a reduction of
the obstruction; it neither proves nor assumes golden inertia.

\section{Two open problems}

We pose the two statements that our working material identifies as
the central open problems of the \(r=5\) study. We claim no proof of
either statement.
...
Theorem~\ref{thm:h4wall} gives its exact kernel-checked saturation form.
...
At the release state audited for this version there are thirty modules
and \(4433\) source lines; no \lean{sorry}, no \lean{admit}, and no axioms
...
repository \lean{README}. The tracked \path{AxiomAudit.lean} runs
\lean{\#print axioms} on the twelve headline declarations in CI; it
...
\item the finite-computation layer is replayed by running
  \path{tools/verify_certificates.py}; the separate script
  \path{tools/verify_scalar.py} independently tests the
  common-sign repair in a four-tuple quotient implementation; the optional
  \lean{--full} flag recomputes the complete \(H_n\) census;
