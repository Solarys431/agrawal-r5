...
Lucas--Carmichael number; no such integer is known. Pomerance's 1984
Baillie--PSW heuristic imposed the same paired Korselt conditions
...
\begin{theorem}[\lean{partition\_forced}]\label{thm:partition}
...
\]
If \(q\) is prime, \(q\mid p-1\), and \(q\mid p'+1\), then \(q\mid2\).
Consequently, for odd \(q\), the two sets
\[
...
(\lean{dvd\_sub\_of\_korselt\_pair}). Hence every prime divisor of
\((p-1)(p+1)\) divides \(m-1\)
(\lean{dvd\_of\_dvd\_sq\_sub\_one}), and therefore
\(n\equiv p\pmod q\). The same argument gives
\(n\equiv p'\pmod q\), so \(p\equiv p'\pmod q\). But the two side
conditions give \(p\equiv1\pmod q\) and \(p'\equiv-1\pmod q\).
Thus \(q\mid2\).
\end{proof}

\begin{remark}[Scope and prior art]
The intermediate congruence
...
\cite{Leng2017}. Theorem~\ref{thm:partition} extracts and kernel-checks
the general odd-\(q\) separation that follows from that mechanism. It is
...
and \(2614\) source lines; no \lean{sorry}, no \lean{admit}, and no axioms
